...
if($acao=="importar" && isset($_FILES["arquivo"])){
	...
	if($extensao != "txt" && $extensao != "csv"){
		...
		header("Location:imp_fornecedor.php");
		exit;
	}
	...
	foreach($linhas as $num_linha => $linha){
		...
		if(count($pt) < 11) continue;
		
		$nome = addslashes($pt[1]);
		$fantasia = addslashes($pt[9]);
		$endereco = addslashes($pt[4]);
		$contato = addslashes($pt[6]);
		$cidade = addslashes($pt[8]);
		
		$sel = mysql_query("SELECT * FROM fornecedores WHERE nome='$nome'");
		if(!mysql_num_rows($sel)){
			$insert = mysql_query("INSERT INTO fornecedores (nome,fantasia,endereco,contato,cidade,estado,cep,cnpj,ie,fone,fax,aceita) VALUES('$nome','$fantasia','$endereco','$contato','$cidade','$pt[7]','$pt[2]','$pt[3]','$pt[5]','$pt[10]','$pt[11]','S')");
	...
	$_SESSION["mensagem"] = "Importacao concluida! $importados fornecedores importados, $erros erros.";
...
<html lang="pt-BR">
<head>
<title>Importar Fornecedores - ERP System</title>
<meta charset="UTF-8">
...
<div class="erp-container-fluid">
    <div class="erp-card">
        <div class="erp-card-header">
            <h1 class="erp-card-title"><i class="fas fa-file-import"></i> Importar Fornecedores</h1>
            <div>
    ...
    <div class="erp-row">
        <div class="erp-col">
            <div class="erp-card">
                ...
                <form method="post" action="imp_fornecedor.php" enctype="multipart/form-data">
                    ...
                    <button type="submit" class="erp-btn erp-btn-success" style="width:100%;">
                        <i class="fas fa-file-import"></i> Importar Fornecedores
                    </button>
                </form>
            </div>
        </div>
            ...
            <div class="erp-card">
                ...
                <div style="background:#f8f9fa;padding:12px;border-radius:8px;margin-bottom:16px;">
                    <code style="display:block;color:#2c3e50;font-size:13px;line-height:1.6;">
                        0. ID (pode ser vazio)<br>
                        1. Nome / Razao Social<br>
                        2. CEP<br>
                        3. CNPJ<br>
                        4. Endereco<br>
                        5. IE<br>
                ...
                <div class="erp-alert erp-alert-info">
                    ...
                    <code style="font-size:12px;">;Fornecedor XYZ Ltda;01234-567;12.345.678/0001-90;Av Principal 456;123456789;Joao Silva;SP;Sao Paulo;XYZ;1133334444;1122223333</code>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="erp-col">
                <div style="padding:12px;background:#d4edda;border-radius:8px;border-left:4px solid #28a745;">
                    ...
                    <ul style="margin:8px 0 0 20px;color:#155724;">
                        <li>Fornecedores novos (nome nao cadastrado)</li>
                        <li>Dados completos e validos</li>
                        <li>Status ativo automatico</li>
                    </ul>
                </div>
            </div>
            ...
            <div class="erp-col">
                <div style="padding:12px;background:#fff3cd;border-radius:8px;border-left:4px solid #ffc107;">
                    ...
                    <ul style="margin:8px 0 0 20px;color:#856404;">
                        <li>Fornecedores duplicados (mesmo nome)</li>
                        <li>Linhas com dados incompletos</li>
                        <li>Erros de formato</li>
                    </ul>
                </div>
            </div>
        </div>

...
function seleciona(id,nome){
	opener.form1.l_instalacao.value=id;
	opener.form1.lo_instalacao.value=nome;
	window.close();
}
windowWidth=320;
windowHeight=300;
		...
		?>
          ...
          <?php
			...
			while($res=mysql_fetch_array($sql)){
				...
				$sql2=mysql_query("SELECT * FROM cliente_entrega WHERE cliente='$res[id]'");		
				$resc=mysql_fetch_array($sql2);

				$resnome=str_replace("'","",$resc["endereco_ins"]);
		?>
		  <tr bgcolor="#FFFFFF" class="texto"> 
            <td>&nbsp;<?php print "$resnome $resc[bairro_ins] $resc[cep_ins] $resc[cidade_ins] $resc[estado_ins]";  ?></td>
            <td width="20" align="center"><a href="#" onClick="return seleciona('<?php print $res["id"]; ?>','<?php print "$resnome $resc[bairro_ins] $resc[cep_ins] $resc[cidade_ins] $resc[estado_ins]";  ?>');"><img src="imagens/icon_14_use.gif" alt="Selecionar" width="14" height="14" border="0"></a></td>
          </tr>
		  <?php
			}
		}

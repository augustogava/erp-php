...
$sql=mysql_query("SELECT * FROM agenda WHERE sit='N' AND data<='$datafim' AND data>='$dataini'");
if(mysql_num_rows($sql)!=0){
	while($res=mysql_fetch_array($sql)){
		$i=substr($res['data'],8,2);
		$i=$i+1-1;
		$Marca[$i]="S";
	}
}
...
print"<table width=100% bgcolor=".$edgecolor." border=0 cellpadding=2 cellspacing=1><tr style=\"color:#$headtextcolor;\">";	// Toon header.
...
while ($dias <= $ultimo_dia){
	...
	print"><a href='".$PageName."?cal_dia=".$print_dia."&cal_mes=".$mes_ativo."&cal_ano=".$ano_ativo.$ExtraURI."'><font size=".$fontsize." color=".$diascolor.">"; if($Marca[$print_dia]=="S"){ print("<b>"); } print $print_dia; if($Marca[$print_dia]=="S"){ print("</b>"); } print "</font></a>";
	print"</td></a>";

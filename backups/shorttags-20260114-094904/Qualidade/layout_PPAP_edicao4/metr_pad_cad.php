...
function pegaDiasMes(mes, ano) {
	var ar = new Array(12);
	ar[0] = 31; // Janeiro
	ar[1] = (ano % 4 == 0) ? 29 : 28; // Fevereiro ano bisexto ou não
	ar[2] = 31; // Março
	ar[3] = 30; // Abril
	ar[4] = 31; // Maio
	ar[5] = 30; // Junho
...
function validade(){
	...
	var datastrday= datastr.substr(0,2); // pega o dia
	var datastrmonth= datastr.substr(3,2); // pega o mês
	var datastryear= datastr.substr(6,10); // pega o ano
	var dataintday= parseInt(datastrday); // transforma o dia para int
	var dataintmonth= parseInt(datastrmonth-1); // transforma o mês para int
	var dataintyear= parseInt(datastryear); // transforma o ano para int

	var diasdomes=pegaDiasMes(dataintmonth,dataintyear);	

	if(diasdomes==29){ // para ano bisexto
		while(frec>=366){
			frecyear++;
			frec-=366;
		}
		for(i=0; i<=11; i++){
			DiasDoMes=pegaDiasMes(i,dataintyear);
			if(frec>=DiasDoMes && frec<=365){
				frecmonth++;
				frec-=DiasDoMes;
				if(frec<DiasDoMes){
				 	frecday=frec;
					window.alert(frecday);
					i=12;
				}
			}
		}
	}else{  // para ano não bisexto
		while(frec>=365){
			frecyear++;
			frec-=365;
		}
		for(i=0; i<=11; i++){ // 0
		DiasDoMes=pegaDiasMes(i,dataintyear); //    31
			if(frec>=DiasDoMes && frec<=364){ // 12 31
				frecmonth++;
				frec-=DiasDoMes;
				if(frec<DiasDoMes){
					i=12;
	...
	frecday+=dataintday;
	frecmonth+=dataintmonth+1;
	frecyear+=dataintyear;
	switch(frecmonth){
		case 0:
			printmonth="01";
		break;
	...
	form1.valc.value=frecday+"/"+frecday+"/"+frecyear;
}
</script>
          ...
          <td><table width="479" border="0" cellpadding="0" cellspacing="3">
            <tr>
              ...
              <td width="302"><span class="textobold">
                <input name="codi" type="text" class="formulario" id="codi" size="10" maxlength="10" value="<? print $codi;?>" <? if($acao=="alt"){ print "readonly"; } ?>>
              </span></td>
            </tr>
            <tr>
              <td class="textobold">Descri&ccedil;&atilde;o:</td>
              <td class="textobold"><input name="desc" type="text" class="formulario" id="desc" size="50" maxlength="50" value="<? print $desc;?>"></td>
            </tr>
            <tr>
              ...
              <td class="textobold"><label>
                ...
                subjetivo
                <input name="tipo" type="radio" value="3" <? if($tipo=="3") print "checked"; ?>>
                atribuído</label></td>
            </tr>
            <tr>
              ...
              <td><font face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000066">
                <select name="unid" class="formulario" id="unid">
				  ...
				  <? $sql4=mysql_query("SELECT * FROM unidades ORDER BY apelido");
				  while ($res4=mysql_fetch_array($sql4)){?>
			 	  <option value="<? print $res4["id"];?>"<? if($res4["id"]==$unid){ print "selected"; }?>><? print $res4["apelido"];?></option>
				<? } ?> 					
               </select>
              </font></td>
            </tr>
            <tr>
              ...
              <td><input name="valn" type="text" class="formulario" id="valn" size="20" maxlength="20" value="<? print $valn;?>"></td>
            </tr>
            <tr>
              <td class="textobold">Toler&acirc;ncia:</td>
              <td><input name="tole" type="text" class="formulario" id="tole" size="20" maxlength="20" value="<? print $tole;?>"></td>
            </tr>
            <tr>
              <td class="textobold">Pad. Utilizado: </td>
              <td><input name="padu" type="text" class="formulario" id="padu" size="20" maxlength="20" value="<? print $padu;?>"></td>
            </tr>
            <tr>
              <td class="textobold">N&uacute;m. de Certificado: </td>
              <td><input name="numc" type="text" class="formulario" id="numc" size="20" maxlength="20" value="<? print $numc;?>"></td>
            </tr>
            <tr>
              <td class="textobold">Freq. de Calibra&ccedil;&atilde;o (dias): </td>
              <td><input name="frec" type="text" class="formulario" id="frec" size="20" maxlength="20" value="<? print $frec;?>"></td>
            </tr>
            <tr>
              <td class="textobold">Data da Calibra&ccedil;&atilde;o:</td>
              <td><input name="datc" type="text" class="formulario" id="datc" size="10" maxlength="10" onKeyPress="return validanum(this, event)" onKeyUp="mdata(this)" onBlur="validade();" value="<? print $datc;?>">&nbsp;&nbsp;<a href="#" class="" onClick="window.open('agenda_pop.php?window_position=metr_pab_cad','','scrollbars=no,width=155,height=138');"><img src="imagens/icon14_cal.gif" width="14" height="14" border="0"></a></td>
            </tr>
            <tr>
              <td class="textobold">Validade da Calibra&ccedil;&atilde;o: </td>
              <td><input name="valc" type="text" class="formulario" id="valc" size="10" maxlength="10" onKeyPress="return validanum(this, event)" onKeyUp="mdata(this)" value="<? print banco2data($valc);?>" readonly=""></td>
            </tr>
            <tr>
              <td class="textobold">Incert. Padr&atilde;o: </td>
              <td><input name="incp" type="text" class="formulario" id="incp" size="20" maxlength="20" value="<? print $incp;?>"></td>
            </tr>
          </table></td>
        </tr>
      </table>

      ...
      <form action="" method="post" name="form1" id="form1" onSubmit="return verifica(this)">
        <table width="594" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <input name="cliente" type="hidden" id="cliente" value="<?= $cli; ?>">
            <input name="acao" type="hidden" id="acao" value="<? print $acao; ?>">
            <input name="id" type="hidden" id="id" value="<? print $idf; ?>"></td>
          </tr>
          <tr>
            <td>
              <select name="titulo" class="formularioselect" id="select2">
                <option value="">Selecione</option>
				<option value="followup" <? if($res["titulo"]=="followup"){ print "selected"; } ?>>Follow-up</option>
				<option value="atualizacao" <? if($res["titulo"]=="atualizacao"){ print "selected"; } ?>>Atualização</option>
				<option value="reclamacao" <? if($res["titulo"]=="reclamacao"){ print "selected"; } ?>>Reclamação</option>
              </select>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td class="textobold"><select name="tipo" class="formularioselect" id="tipo">
			  ...
			  while($res2=mysql_fetch_array($sql2)){
			   ?>
			   <option value="<? print $res2["id"]; ?>" <? if($res["tipo"]==$res2["id"]){ print "selected"; } ?>><? print $res2["nome"]; ?></option>
			   <? } ?>
            </select>            </td>
          </tr>
          <tr>
...
?>
          ...
          <tr>
            <td class="textobold"><select name="contato" class="formularioselect" id="select3">
			  ...
			  while($res2=mysql_fetch_array($sql2)){
			   ?>
              <option value="<? print $res2["id"]; ?>" <? if($res["contato"]==$res2["id"]){ print "selected"; } ?>><? print $res2["nome"]; ?></option>
              <? } ?>
            </select></td>
          </tr>
          <tr>
          ...
          <tr>
            <td><textarea name="descricao" rows="6" wrap="VIRTUAL" class="formularioselect" id="textarea"  onFocus="enterativa=0;" onBlur="enterativa=1;"><? print $res["descricao"]; ?></textarea></td>
          </tr>
          <tr>
            <td align="center">&nbsp;</td>
          </tr>

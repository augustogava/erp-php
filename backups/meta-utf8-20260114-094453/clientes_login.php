...
if($acao=="alt"){
	...
	if(strlen($senha)<6){
		...
		$sql=mysql_query("UPDATE cliente_login SET login='$login',senha='$senha',nivel='$nivel',sit='$sit',perm='$porra',primeiro='$primeiro' WHERE cliente='$id'");
		if($sql){
...
function verifica(cad){
	...
	if(cad.senha.value!=''){
		...
		if(cad.senha2.value!=cad.senha.value){
			alert('A senha e a confirma��o n�o conferem');
			cad.senha2.value='';
			cad.senha2.focus();
			return false;
		}

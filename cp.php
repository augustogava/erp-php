...
$block=false;
if($acao=="alt"){
	$sql=mysql_query("SELECT * FROM cp WHERE id='$id'");
	...
	$sqlb=mysql_query("SELECT * FROM cp_itens WHERE conta='$id' AND pago='N'");
...
function verifica(cad){
	...
	<?php }else{ ?>
	alert('Este lancamento nao pode ser alterado pois ja existem duplicatas pagas');	
	<?php } ?>
	return false
}
</script>
...
<div class="erp-container-fluid">
    <div class="erp-card">
        <div class="erp-card-header">
            <h1 class="erp-card-title"><i class="fas fa-file-invoice-dollar"></i> Contas a Pagar</h1>
            <div style="display:flex;gap:8px;">
                <a href="cp_aberto.php" class="erp-btn erp-btn-outline">
                    <i class="fas fa-list"></i> CP Aberto
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="erp-card">
        <div class="erp-card-header">
            <h3><i class="fas fa-edit"></i> <?php if($acao=="inc"){ echo "Incluir"; }else{ echo "Alterar"; } ?> Conta a Pagar</h3>
        </div>
...
<?php if($acao=="alt"){ ?>
            <div class="erp-table-container" style="margin-top:30px;">
                ...
                <table class="erp-table">
                    ...
                    <tbody>
<?php
$sql=mysql_query("SELECT * FROM cp_itens WHERE conta='$id' ORDER BY vencimento ASC");
$tp=0;
$tp2=0;
while($res=mysql_fetch_array($sql)){
    if($res["pago"]=="S"){
...
<?php } ?>
                            ...
                            <td class="erp-text-center">
                                <a href="#" onclick="return abre('cp_hist.php?id=<?php echo $res["id"]?>','a','width=470,height=300,scrollbars=1');">
<?php if($res["pago"]=="N"){ ?>
...
<?php if($res["pago"]=="S" and $sit!="C"){ ?>
                                <a href="#" onclick="return pergunta('Deseja desfazer o pagamento desta parcela?','cp_sql.php?acao=desf&ct=<?php echo $id?>&id=<?php echo $res["id"]?>');" class="erp-table-action" style="color:#e74c3c;">
                ...
                <table class="erp-table">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td><strong style="color:#e74c3c;">R$ <?php echo banco2valor($valor2-$tp2)?></strong></td>
                            <td>
<?php 
$sit_class = "info";
$sit_text = "";
...
?>
            ...
            <div style="margin-top:20px;display:flex;gap:10px;justify-content:flex-end;">
                <button type="button" class="erp-btn erp-btn-outline" onclick="window.location='cp_aberto.php'">
                    <i class="fas fa-arrow-left"></i> Voltar
                </button>
                <button type="button" class="erp-btn erp-btn-danger" onclick="return <?php if($sit=="C"){ echo "alert('Esta conta ja esta cancelada');"; }else{ echo "pergunta('Deseja cancelar esta conta?','cp_sql.php?acao=can&id=$id');"; } ?>">

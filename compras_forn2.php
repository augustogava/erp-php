...
<script>
function seleciona(nome,id,valor,data){
	opener.form1.nome.value=nome;
	opener.form1.fornecedor.value=id;
	opener.form1.ucv.value=valor;
	opener.form1.ucd.value=data;
	window.close();
}
windowWidth=320;
windowHeight=300;
    ...
    <td><table width="300" border="0" cellpadding="0" cellspacing="1" bgcolor="#003366">
        ...
        <?php
			...
			while($res=mysql_fetch_array($sql)){
				...
				$valor="0,00";
				$data="";
				$sqlv=mysql_query("SELECT SUM((compras_list.qtd * compras_list.unitario) - ((compras_list.qtd * compras_list.unitario) * compras_list.desconto / 100)) AS valor FROM compras,compras_list WHERE compras.fornecedor='$res[id]' AND compras_list.compra=compras.id ORDER BY compras.emissao DESC");
				if(mysql_num_rows($sqlv)){
					$resv=mysql_fetch_array($sqlv);
					$valor=banco2valor($resv["valor"]);
				}
				$sqlv=mysql_query("SELECT compras.emissao AS data FROM compras,compras_list WHERE compras.fornecedor='$res[id]' AND compras_list.compra=compras.id ORDER BY compras.emissao DESC");
				if(mysql_num_rows($sqlv)){
					$resv=mysql_fetch_array($sqlv);
					$data=banco2data($resv["data"]);
				}
		?>
		<tr bgcolor="#FFFFFF" class="texto"> 
          <td>&nbsp;<?php print $res["nome"]; ?></td>
          <td width="20" align="center"><a href="#" onClick="return seleciona('<?php print $res["nome"]; ?>','<?php print $res["id"]; ?>','<?php echo  $valor; ?>','<?php echo  $data; ?>');"><img src="imagens/icon_14_use.gif" alt="Selecionar" width="14" height="14" border="0"></a></td>
        </tr>
		<?php
			}
		}

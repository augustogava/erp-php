...
if(!empty($bcod)){
	$cond.=" AND codigo LIKE '%$bcod%' ";
}

?>
<html>
<head>
<title>Busca de Ensaio</title>
...
function marca(id,codigo){
		opener.form1.codens.value=codigo;
		opener.form1.idcodens.value=id;
	window.close();
}
function fec(){
	window.close();
...
<table width="307" border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#999999">
  ...
  <tr class="textoboldbranco">
    <td width="95">C&oacute;digo</td>
    <td width="197">Descri&ccedil;&atilde;o</td>
  </tr>
	<?php
	$sql=mysql_query("SELECT * FROM ensaio WHERE 1 $cond ORDER BY codigo ASC");
	if(mysql_num_rows($sql)==0){
	?>
  <tr bgcolor="#FFFFFF">
    <td colspan="2" align="center" class="textopretobold">NENHUM ENSAIO CADASTRADO</td>
  </tr>
	<?php
	}else{
		//BLOCO PAGINACAO
		...
		if($results_tot>$maxpag){
			...
			$sql=mysql_query("SELECT * FROM ensaio WHERE 1 $cond ORDER BY codigo ASC LIMIT $param, $maxpag");
...
<a href="javascript:window.close();" onClick="marca('<?php print $res["id"]; ?>','<?php echo $res["codigo"]." - ".$res["descricao"];?>');"></a>
<?php if($wpaginar){ ?>
            ...
            <tr>
              <td align="center">
				...
				if($wp>1){
					$antz=true;
				?>
                      <a href="<?php print "rec_plac_lise.php?wp=$pg_anterior&bdesc=$bdesc&bcod=$bcod"; ?>" class="texto">
                      <?php } ?>
            ...
            Anterior</a>
				...
				?>
                    <td align="center">
                      ...
                      <a href="<?php print "rec_plac_lise.php?wp=$link_impressos&bdesc=$bdesc&bcod=$bcod"; ?>" class="texto">
                      <?php } ?>
				...
				?>
                    <td>
                      ...
                      <a href="<?php print "rec_plac_lise.php?wp=$pg_proxima&bdesc=$bdesc&bcod=$bcod"; ?>" class="texto">
                      <?php } ?>

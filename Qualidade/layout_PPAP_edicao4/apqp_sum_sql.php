...
if($acao=="s1"){
	...
	if(isset($ap1)){
		...
		if(mysql_num_rows($sql)){
			...
			$sql=mysql_query("UPDATE apqp_cron SET resp='$tap11', fim=NOW(), perc='100' WHERE peca='$pc' AND (ativ='Sumário e Aprovação do APQP' or ativ='Sumario de Aprovação do APQP (Validação final)')") or erp_db_fail();

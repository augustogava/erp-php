...
if(!empty($bapelido)){
	$cond.=" AND apelido LIKE '%$bapelido%' ";
}

?>
<html>
<head>
<title>Busca de Unidade de Medida</title>
  ...
  <tr>
    <td colspan="2" bgcolor="#FFFFFF" class="texto style1"><form name="form1" method="post" action="">
      <table width="300" border="0" cellspacing="1" cellpadding="3">
        ...
        <tr>
          <td width="52" class="textobold">Nome:</td>
          <td width="248"><input name="bapelido" type="text" class="formulario" id="bapelido" size="15">
  ...
  <tr bgcolor="#FFFFFF">
	...
	<?php
		...
		if($results_tot>$maxpag){
			...
			$sql=mysql_query("SELECT * FROM unidades WHERE 1 $cond ORDER BY apelido ASC LIMIT $param, $maxpag");

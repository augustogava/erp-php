...
$pdf->Cell(70);
$pdf->Cell(0, 18, 'BERICHT ZUR FREIGABE DES AUSSEHENS');
$pdf->SetFont('Arial','',8);
$pdf->SetXY(5, 18);
$pdf->MultiCell(60,5,"Kundenteilenummer \n $res[nomecli]",1);
$pdf->SetXY(65, 18);
$pdf->MultiCell(70,5,"Zeichnungs-nummer \n $res[pecacli]",1);
$pdf->SetXY(135, 18);
$pdf->MultiCell(60,5,"Lieferantenname \n $res[desenhoc]",1);
$pdf->SetXY(195, 18);
$pdf->MultiCell(95,5,"Anwendung \n $res[aplicacao]",1);
$pdf->SetXY(260, 5);
...
//linha 2
$pdf->SetXY(5, 28);
$pdf->MultiCell(75,5,"Teilename \n $res[nome]",1);
$pdf->SetXY(80, 28);
$pdf->MultiCell(75,5,"Einkäufer-Kernnummer \n $resp[comprador]",1);
$pdf->SetXY(155, 28);
$pdf->MultiCell(105,5,"E/A Stand \n $res[niveleng]",1);
$pdf->SetXY(260, 28);
$pdf->MultiCell(30,5,"Datum \n ".banco2data($res["dteng"])."",1);
//linha3
$pdf->SetXY(5, 38);
$pdf->MultiCell(100,5,"Names des Lieferanten \n a$rese[razao]",1);
$pdf->SetXY(105, 38);
$pdf->MultiCell(50,5,"Lieferantenteilenummer \n  $res[numero] - $res[rev]",1);
$pdf->SetXY(155,38);
$pdf->MultiCell(105,5,"Herstellungsort \n $resp[local]",1);
$pdf->SetXY(260, 38);
$pdf->MultiCell(30,5,"Lieferanten Kenn. \n $resp[seilaquecodigoehesse]",1);
//linha4
	//coluna 1
	$pdf->SetXY(5, 48);
...
//linha 5 
	$pdf->SetXY(5, 70);
	$pdf->SetFont('Arial','B',8);
	$pdf->MultiCell(285,5,"EVALUIERUNG DES AUSEHENS",1,'C');
	$pdf->SetXY(5, 75);
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(185,5,"Lieferanteninformation bezüglich der (Ausehens-) Ursprünge und der Oberflächenstruktur",1,'C');
	$pdf->SetXY(5, 80);
	...
	//tabela direta
	$pdf->SetXY(190, 75);
	$pdf->MultiCell(40,10,"Vorläufige Oberfläch.",1,'C');
	$pdf->SetXY(230, 75);
	$pdf->MultiCell(60,5,"Unterschrift des \n Kundenvertreters und datum",1,'C');
	$pdf->SetXY(190, 85);
	$pdf->MultiCell(40,10,"Korrektur und Weiterleitung",1,'C');
	$pdf->SetXY(230, 85);
	$pdf->MultiCell(60,10," ",1,'C');
	$pdf->SetXY(190, 95);
	$pdf->MultiCell(40,10,"Korrektur und wiedervorlage",1,'C');
	$pdf->SetXY(230, 95);
	$pdf->MultiCell(60,10,"",1,'C');
	$pdf->SetXY(190, 105);
	$pdf->MultiCell(40,5,"Oberfläche genehmigt",1,'C');
	$pdf->SetXY(230, 105);
...
//linha 6
	...
	$pdf->SetXY(5, 112);
	$pdf->SetFont('Arial','B',8);
	$pdf->MultiCell(15,5,"Farbcode \n ",1,'C');
	$pdf->SetXY(20, 112);
	$pdf->MultiCell(35,10,"farbwerte(Tristimulus)",1,'C');
	$pdf->SetXY(55, 112);
	$pdf->MultiCell(15,5,"Muster Nummer \n ",1,'C');
	$pdf->SetXY(70, 112);
	$pdf->MultiCell(20,5,"Muster Datum \n ",1,'C');
	$pdf->SetXY(90, 112);
	$pdf->MultiCell(25,15,"Material Typ",1,'C');
	$pdf->SetXY(115, 112);
	$pdf->MultiCell(30,15,"Material Quelle",1,'C');
	$pdf->SetXY(145, 112);
	$pdf->MultiCell(35,10,"Farbton",1,'C');
	$pdf->SetXY(180, 112);
	$pdf->MultiCell(20,10,"Grad",1,'C');
	$pdf->SetXY(200, 112);
	$pdf->MultiCell(20,10,"Chroma",1,'C');
	$pdf->SetXY(220, 112);
	$pdf->MultiCell(20,10,"Glanz",1,'C');
	$pdf->SetXY(240, 112);
	$pdf->MultiCell(20,5,"Metalll cohe Brillanz",1,'C');
	$pdf->SetXY(260, 112);
	$pdf->MultiCell(15,5,"Chargen- nummer der Farbe",1,'C');
	$pdf->SetXY(275, 112);
	$pdf->MultiCell(15,5,"Entscheidung für Teil",1,'C');
	//linha 2
	$pdf->SetXY(20, 122);
	$pdf->SetFont('Arial','',7);
	...
	$pdf->SetXY(145, 122);
	$pdf->MultiCell(10,5,"Rot",1,'C');
	$pdf->SetXY(155, 122);
	$pdf->MultiCell(8,5,"Geb",1,'C');
	$pdf->SetXY(163, 122);
	$pdf->MultiCell(8,5,"Grün",1,'C');
	$pdf->SetXY(171, 122);
	$pdf->MultiCell(9,5,"Bleu",1,'C');
	$pdf->SetXY(180, 122);
	$pdf->MultiCell(10,5,"Hell",1,'C');
	$pdf->SetXY(190, 122);
	$pdf->MultiCell(10,5,"Dutel",1,'C');
	$pdf->SetXY(200, 122);
	$pdf->MultiCell(10,5,"Grau",1,'C');
	$pdf->SetXY(210, 122);
	$pdf->MultiCell(10,5,"Blent",1,'C');
	$pdf->SetXY(220, 122);
	$pdf->MultiCell(10,5,"Hoch",1,'C');
	$pdf->SetXY(230, 122);
	$pdf->MultiCell(10,5,"Matt",1,'C');
	$pdf->SetXY(240, 122);
	$pdf->MultiCell(10,5,"Hoch",1,'C');
	$pdf->SetXY(250, 122);
	$pdf->MultiCell(10,5,"matt",1,'C');

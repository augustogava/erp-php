<?php
$npc=$_SESSION["npc"];
$sql=mysql_query("SELECT *,apqp_pc.nome AS nome, clientes.fantasia AS nomecli FROM apqp_pc,clientes WHERE apqp_pc.id='$pc' AND apqp_pc.cliente=clientes.id");
$res=mysql_fetch_array($sql);
if(!mysql_num_rows($sql)) exit;
$sql=mysql_query("SELECT * FROM empresa");
if(mysql_num_rows($sql)) $rese=mysql_fetch_array($sql);
$sql=mysql_query("SELECT * FROM apqp_endi WHERE peca='$pc'");
if(mysql_num_rows($sql)) $resp=mysql_fetch_array($sql);

$eng=$res["niveleng"];
$proj=$res["nivelproj"];
$resultado=$resp["result"];
$cod_forn=$res["cod_forn"];
$pdf->AddPage();
$pg=1;
$razao=$rese["razao"];
$cliente=$res["pecacli"];
$numero=$res["numero"];
$rev=$res[niveleng]." - ".banco2data($res["dteng"]);
$local=$resp["local"];
$num=$res["numero"]." - ".$res["rev"];
$nome=$res["nome"];
$pdf->Image('empresa_logo/logo.jpg',5,1,20);  // logo qualitmanager
$pdf->Image('imagens/logo_chrysler.jpg',25,15,25,3);
$pdf->Image('imagens/logo_ford.jpg',50,12.5,13);
$pdf->Image('imagens/logo_gm.jpg',64,11.5,6);
$pdf->SetXY(40, 0);
$pdf->SetFont('Arial','B',12);
$pdf->Cell(40);
$pdf->Cell(0, 8, 'ProduktionscTeilcZustimmung');
$pdf->SetXY(40, 5);
$pdf->Cell(48);
$pdf->Cell(0, 8, 'MaßcTestergebnisse');
$pdf->SetFont('Arial','B',8);
$pdf->SetXY(180, 5);
$pdf->MultiCell(40,5,"PPAP Nr. $numero \n Blatt: $pg");
$pdf->SetXY(5, 18);
$pdf->MultiCell(104,10,"",1);
$pdf->SetXY(5, 18);
$pdf->SetFont('Arial','',8);
$pdf->MultiCell(110,5,"Organisation: $razao");
$pdf->SetXY(5, 23);
$pdf->MultiCell(110,5,"Lieferant/Verkäufe Code: $cod_forn");
$pdf->SetXY(109, 18);
$pdf->MultiCell(96,25,"",1);
$pdf->SetXY(109, 18);
$pdf->MultiCell(90,5,"Teilnummer (Kunde): $cliente");
//linha 2
$pdf->SetXY(5, 28);
$pdf->MultiCell(104,15,"",1);
$pdf->SetXY(5, 28);
$pdf->MultiCell(110,5,"Kontrollen-Service: \n $local");
$pdf->SetXY(109, 23);
$pdf->MultiCell(90,5,"Teilename: $nome");
$pdf->SetXY(109, 28);
$pdf->MultiCell(90,5,"DesigncRekordcÄnderungscNiveau: $proj");
$pdf->SetXY(109, 33);
$pdf->MultiCell(90,5,"ÄnderungscDokumente Enginnering: $eng");

//linha 3
$pdf->SetFont('Arial','B',6.5);
$pdf->SetXY(5, 43);
$pdf->MultiCell(12,10,"Einzelteil",1,'C');
$pdf->SetFont('Arial','B',7.5);
$pdf->SetXY(17, 43);
$pdf->MultiCell(43,10,"Maß/Spezifikation",1,'C');
$pdf->SetXY(60, 43);
$pdf->MultiCell(25,5,"Begrenzungen",1,'C');
$pdf->SetXY(60, 48);
$pdf->MultiCell(12.5,5,"LIE",1,'C');
$pdf->SetXY(72.5, 48);
$pdf->MultiCell(12.5,5,"LSE",1,'C');
$pdf->SetXY(85, 43);
$pdf->MultiCell(12,5,"Prüftermin",1,'C');
$pdf->SetXY(97, 43);
$pdf->MultiCell(12,5,"Quant.\nPrüfte",1,'C');
$pdf->SetXY(109, 43);
$pdf->MultiCell(80,5,"Testergebnisse (Datum)",1,'C');
$pdf->SetXY(109, 48);
$pdf->MultiCell(40,5,"Durch Organisation",1,'C');
$pdf->SetXY(149, 48);
$pdf->MultiCell(40,5,"Durch Kunden",1,'C');
$pdf->SetXY(189, 43);
$pdf->MultiCell(8,10,"O.K.",1,'C');
$pdf->SetXY(197, 43);
$pdf->MultiCell(8,10,"",1,'C');
$pdf->SetXY(196.5, 43);
$pdf->MultiCell(9,5,"Nicht O.K.",0,'C');

//linha 4
$y=53;
$sql=mysql_query("SELECT ensaio1.*,car.numero,car.descricao,car.simbolo,car.espec,car.tipo,car.lie,car.lse FROM apqp_car AS car, apqp_endi AS ensaio, apqp_endil AS ensaio1 WHERE ensaio1.car=car.id AND ensaio.peca='$pc' AND ensaio.id=ensaio1.ensaio ORDER BY car.numero ASC");
if(mysql_num_rows($sql)){
	while($res=mysql_fetch_array($sql)){
		if($y>=250){
				// desenvolvedor
				$pdf->SetFont('Arial','B',5);  
				$pdf->SetXY(168,269);
				$pdf->MultiCell(50,7,"Powered by www.qualitymanager.com.br");
			$y=53;
			$pg++;
			$pdf->AddPage();
			$pdf->Image('empresa_logo/logo.jpg',5,1,20);  // logo qualitmanager
			$pdf->Image('imagens/logo_chrysler.jpg',25,15,25,3);
			$pdf->Image('imagens/logo_ford.jpg',50,12.5,13);
			$pdf->Image('imagens/logo_gm.jpg',64,11.5,6);
			$pdf->SetXY(40, 0);
			$pdf->SetFont('Arial','B',12);
			$pdf->Cell(40);
			$pdf->Cell(0, 8, 'ProduktionscTeilcZustimmung');
			$pdf->SetXY(40, 5);
			$pdf->Cell(48);
			$pdf->Cell(0, 8, 'MaßcTestergebnisse');
			$pdf->SetFont('Arial','B',8);
			$pdf->SetXY(180, 5);
			$pdf->MultiCell(40,5,"PPAP Nr. $numero \n Blatt: $pg");
			$pdf->SetXY(5, 18);
			$pdf->MultiCell(104,10,"",1);
			$pdf->SetXY(5, 18);
			$pdf->SetFont('Arial','',8);
			$pdf->MultiCell(110,5,"Organisation: $razao");
			$pdf->SetXY(5, 23);
			$pdf->MultiCell(110,5,"Lieferant/Verkäufe Code: $cod_forn");
			$pdf->SetXY(109, 18);
			$pdf->MultiCell(96,25,"",1);
			$pdf->SetXY(109, 18);
			$pdf->MultiCell(90,5,"Teilnummer (Kunde): $cliente");
			//linha 2
			$pdf->SetXY(5, 28);
			$pdf->MultiCell(104,15,"",1);
			$pdf->SetXY(5, 28);
			$pdf->MultiCell(110,5,"Kontrollen-Service: \n $local");
			$pdf->SetXY(109, 23);
			$pdf->MultiCell(90,5,"Teilename: $nome");
			$pdf->SetXY(109, 28);
			$pdf->MultiCell(90,5,"DesigncRekordcÄnderungscNiveau: $proj");
			$pdf->SetXY(109, 33);
			$pdf->MultiCell(90,5,"ÄnderungscDokumente Enginnering: $eng");
			
			//linha 3
			$pdf->SetFont('Arial','B',6.5);
			$pdf->SetXY(5, 43);
			$pdf->MultiCell(12,10,"Einzelteil",1,'C');
			$pdf->SetFont('Arial','B',7.5);
			$pdf->SetXY(17, 43);
			$pdf->MultiCell(43,10,"Maß/Spezifikation",1,'C');
			$pdf->SetXY(60, 43);
			$pdf->MultiCell(25,5,"Begrenzungen",1,'C');
			$pdf->SetXY(60, 48);
			$pdf->MultiCell(12.5,5,"LIE",1,'C');
			$pdf->SetXY(72.5, 48);
			$pdf->MultiCell(12.5,5,"LSE",1,'C');
			$pdf->SetXY(85, 43);
			$pdf->MultiCell(12,5,"Prüftermin",1,'C');
			$pdf->SetXY(97, 43);
			$pdf->MultiCell(12,5,"Quant.\nPrüfte",1,'C');
			$pdf->SetXY(109, 43);
			$pdf->MultiCell(80,5,"Testergebnisse (Datum)",1,'C');
			$pdf->SetXY(109, 48);
			$pdf->MultiCell(40,5,"Durch Organisation",1,'C');
			$pdf->SetXY(149, 48);
			$pdf->MultiCell(40,5,"Durch Kunden",1,'C');
			$pdf->SetXY(189, 43);
			$pdf->MultiCell(8,10,"O.K.",1,'C');
			$pdf->SetXY(197, 43);
			$pdf->MultiCell(8,10,"",1,'C');
			$pdf->SetXY(196.5, 43);
			$pdf->MultiCell(9,5,"Nicht O.K.",0,'C');
			$pdf->SetFont('Arial','B',6.5);
		}
		$tam1[0]=flinha($res["numero"],12);
		$tam1[1]=flinha("$res[descricao] - $res[espec]",43);	
		$tam1[2]=flinha("$res[lie] / $res[lse]",25);
		$tam1[3]=flinha("$data_teste",13);
		$tam1[4]=flinha("$res[quant_test]",12);
		$tam1[5]=1;
		$tam1[6]=flinha($res["forn"],40);
		if($tam1[6]==0){$tam1[6]=1;}
		$tam1[7]=flinha($res["cli"],40);
		if($tam1[7]==0){$tam1[7]=1;}
		$tam1[8]=1;
		$maxu=max($tam1);
		for($i=0;$i<=11;$i++){
			$ql[$i]=$maxu - $tam1[$i];
		}
		$w=5;
		$pdf->SetFont('Arial','',6);
		$pdf->SetXY(5, $y);
		$pdf->MultiCell(12,$w,$res["numero"],0,'C');
		$pdf->SetXY(17,$y);
		$pdf->MultiCell(43,$w,"$res[descricao] - $res[espec]",0);
		if(!empty($res["lie"])){
			$pdf->SetXY(60,$y);
			$pdf->MultiCell(12.5,$w,"$res[lie]",0,'C');
		}
		if(!empty($res["lse"])){
			$pdf->SetXY(72.5,$y);
			$pdf->MultiCell(12.5,$w,"$res[lie]",0,'C');
		}
		if(!empty($res["data_t"])){
			$data_teste=banco2data($res["data_t"]);
			$pdf->SetXY(84.5,$y);
			$pdf->MultiCell(13,$w,"$data_teste",0,'C');
		}
		if(!empty($res["quant_test"])){
			$pdf->SetXY(97,$y);
			$pdf->MultiCell(12,$w,"$res[quant_test]",0,'C');
		}
		$pdf->SetXY(109, $y);
		$pdf->MultiCell(40,$w,$res["forn"],0);
		$pdf->SetXY(149, $y);
		$pdf->MultiCell(40,$w,$res["cli"],0);		
		$pdf->SetXY(189, $y);
		if($res["ok"]=="S"){$ok="X";}else{$ok="";}
		$pdf->MultiCell(8,$w,$ok,0,'C');
		if($res["ok"]=="N"){$ok="X";}else{$ok="";}
		$pdf->SetXY(197, $y);
		$pdf->MultiCell(8,$w,$ok,0,'C');
		$y=($maxu*5)+$y;
		$pdf->Line(5,$y,205,$y);
		$pdf->Line(5,253,205,253);
		// linhas das colunas
		$pdf->Line(5,53,5,253);
		$pdf->Line(17,53,17,253);
		$pdf->Line(60,53,60,253);
		$pdf->Line(72.5,53,72.5,253);		
		$pdf->Line(85,53,85,253);
		$pdf->Line(97,53,97,253);
		$pdf->Line(109,53,109,253);
		$pdf->Line(149,53,149,253);
		$pdf->Line(189,53,189,253);
		$pdf->Line(197,53,197,253);
		$pdf->Line(205,53,205,253);
	}
	// desenvolvedor
	$pdf->SetFont('Arial','B',5);  
	$pdf->SetXY(168,269);
	$pdf->MultiCell(50,7,"Powered by www.qualitymanager.com.br");
}
//ultima linha
$pdf->SetFont('Arial','',6);
$pdf->SetXY(15, 253.5);
$pdf->MultiCell(8,3,"March  2006",'C');

$pdf->SetFont('Arial','B',7);
$pdf->SetXY(30, 254);
$pdf->MultiCell(20,5,"CFG-1003",'C');

$pdf->SetFont('Arial','',7);
$pdf->SetXY(95, 254);
$pdf->MultiCell(110,5,"Die allgemeinen Informationen, wie nicht für das ganzes Resultat der Tests nicht annehmbar sind.",1);

$y=260;
$pdf->SetFont('Arial','',7);
$pdf->SetXY(5, $y);
$pdf->MultiCell(82,5,"Genehmigt für \n $resp[quem]",1,'');
$pdf->SetXY(87, $y);
$pdf->MultiCell(20,5,"Datum \n ".banco2data($resp["dtquem"])."",1);
$pdf->SetXY(107, $y);
$pdf->MultiCell(80,5,"Unterschrift des Lieferanten \n $resp[rep]",1,'');
$pdf->SetXY(187, $y);
$pdf->MultiCell(18,5,"Datum \n ".banco2data($resp["dtrep"])."",1);


//fim
?>

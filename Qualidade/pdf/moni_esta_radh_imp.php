...
$pdf->Line(24,19,205,19);
    ...
    // Título
	$pdf->SetXY(5,12);
	$pdf->MultiCell(205,7,"RELATÓRIO DE ACESSO POR DATA E HORA",0,"C");
...
$pdf->SetXY(5,21);  
$pdf->MultiCell(205,5,"Horário de $hora_i à $hora_f",0,"C");  // caixa de texto sem borda com 30 caracteres e 5 de altura
...
// 2ª linha  ////////////////////////////////////////////////////////
			$pdf->SetXY(5,27);
			$pdf->MultiCell(200,5,"Data: $data",0,"L");
...
// tabela 6
$pdf->SetXY(113,32);
$pdf->MultiCell(75,5,"Página",1,"C");
					
// tabela 7
$pdf->SetXY(188,32);
$pdf->MultiCell(17,5,"IP",1,"C");
...
$sql=mysql_query("SELECT * FROM log WHERE hora>='$hora_i' AND hora<='$hora_f' AND data='$data' ORDER BY user ASC");

// alerta caso não exista registro armazenado
if(!mysql_num_rows($sql)){
	$_SESSION["mensagem"]="Não achou nenhum Registro";
	header("Location:../moni_esta_radh.php");
	exit;
}
...
while($res=mysql_fetch_array($sql)){
		// - - - - -- - - - - - -  -- - - - - - - - - - - - - - - - - 
			if($y>=265){
				...
				$pdf->SetXY(5,21);  
				$pdf->MultiCell(205,5,"Horário de $hora_i à $hora_f",0,"C");
							// data do dia no canto direito
							$datahj=banco2data($hj);    // banco2data tabela 3
							$pdf->SetFont('Arial','B',5);  
							$pdf->SetXY(180,21);
		...
		$pdf->SetFont('Arial','',6);	
			...
			$pdf->MultiCell(32,5,"$res2[nome]");
			}
			$nome_ant=$res2[nome];
			if($y==38 && $pag>1){
			$pdf->MultiCell(32,5,"$nome_ant");
			}
			...
			$pdf->SetXY(64,$y);
			$pdf->MultiCell(14,5,"$nome_acao",0,"L");    
			...
			// tabela 6  
			$pdf->SetXY(113,$y);
			$pdf->MultiCell(75,5,"$res[pagina]",0,"L");			

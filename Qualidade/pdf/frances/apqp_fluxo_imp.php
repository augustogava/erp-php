...
$pc=$_SESSION["mpc"];
$npc=$_SESSION["npc"];
$sql=mysql_query("SELECT *,apqp_pc.nome AS nome, clientes.fantasia AS nomecli FROM apqp_pc,clientes WHERE apqp_pc.id='".$_GET["pc"]."' AND apqp_pc.cliente=clientes.id");
...
$pdf->Cell(70);
$pdf->Cell(0, 18, 'DIAGRAMA DE FLUXO DO PROCESSO');
...
if(mysql_num_rows($sql)){
	while($res=mysql_fetch_array($sql)){
		...
		$tam[0]=1;
		$tam[1]=ceil(strlen($resop["numero"])/8);
		if($tam[1]==0){$tam[1]=1;}
		$tam[2]=ceil(strlen($resop["descricao"])/40);
		if($tam[2]==0){$tam[2]=1;}
		$tam[3]=ceil(strlen($resop["obs"])/50);
		if($tam[3]==0){$tam[3]=1;}
		$max=max($tam);
		for($i=0;$i<=3;$i++){
			if($tam[$i]==1){
...
//fim
if(empty($tira)){
$pdf->Output('apqp_fluxo_imp.pdf','I');
}
?>

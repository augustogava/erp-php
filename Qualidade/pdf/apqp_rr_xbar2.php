<?php
if($tipo=="msa"){
	$sql_x=mysql_query("SELECT * FROM msa_rr WHERE id='$resp[id]'");
}else{
	$sql_x=mysql_query("SELECT * FROM apqp_rr WHERE id='$resp[id]'");
}
$res_x=mysql_fetch_array($sql_x);
// Some data
$datay="";
$data4y="";
$data3y="";
...
$databarx="";
$data2y="";
for($i=1;$i<=$res_x["nop"];$i++){
	for($j=1;$j<=$res_x["npc"];$j++){
		$datay[]=$res_x["average"];
		$data4y[]=$res_x["uclx"];
		$data3y[]=$res_x["lcl"];
		if($i==1) $lbl="a";
...
// Output line
$graph->Stroke("imagens_fotos/rr_xgraf$num_graf.png");
?>
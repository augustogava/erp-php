...
if(mysql_num_rows($sql)){
	...
	while($res=mysql_fetch_array($sql)){
		...
		$tam[0]=flinha($res["numero"],4);
		if($tam[0]==0){$tam[0]=1;}
		$tam[1]=flinha($res["descricao"],30);
		if($tam[1]==0){$tam[1]=1;}
		$tam[2]=flinha($res["espec"],20);
		if($tam[2]==0){$tam[2]=1;}
		$tam[3]=flinha($res["tecnicas"],16);
		if($tam[3]==0){$tam[3]=1;}
		$tam[4]=flinha($res["tamanho"],8);
		if($tam[4]==0){$tam[4]=1;}
		$tam[5]=flinha($res["freq"],8);
		if($tam[5]==0){$tam[5]=1;}
		$tam[6]=flinha($res["metodo"],16);
		if($tam[6]==0){$tam[6]=1;}
		$tam[7]=flinha($res["reacao"],30);
		if($tam[7]==0){$tam[7]=1;}
		$maxi=max($tam);
		for($i=0;$i<=11;$i++){
			$ql[$i]=$maxi - $tam[$i];

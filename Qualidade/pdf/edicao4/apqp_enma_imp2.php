...
$pdf->Cell(35);
$pdf->Cell(0, 8, 'Aprovação de Peça de Produção');
$pdf->SetXY(40, 5);
$pdf->Cell(30);
$pdf->Cell(0, 8, 'Resultados dos Ensaios de Materiais');
...
$pdf->SetXY(5, 18);
$pdf->SetFont('Arial','',8);
$pdf->MultiCell(110,5,"Organização: $razao");
$pdf->SetXY(5, 23);
$pdf->MultiCell(110,5,"Código do Fornecedor/Vendedor: $cod_forn");
$pdf->SetXY(115, 18);
$pdf->MultiCell(90,25,"",1);
$pdf->SetXY(115, 18);
$pdf->MultiCell(90,5,"Número da Peça: $num");
//linha 2
$pdf->SetXY(5, 28);
$pdf->MultiCell(110,15,"",1);
$pdf->SetXY(5, 28);
$pdf->MultiCell(110,5,"Fornecedor do material: $resp[forn_mat]");
$pdf->SetXY(5, 33);
$pdf->MultiCell(137,5,"Fornecedor especificado pelo cliente / Código Vendedor: $resp[forncod]");

$pdf->SetFont('Arial','',6);
$pdf->SetXY(5, 38);
$pdf->MultiCell(110,5,"* Quando é requerida a aprovação da fonte, incluir o código do fornecedor.");

$pdf->SetFont('Arial','',8);
$pdf->SetXY(115, 23);
$pdf->MultiCell(90,5,"Nome da Peça: $nome");
$pdf->SetXY(115, 28);
$pdf->MultiCell(90,5,"Nível de alteraçao do projeto: $proj");
$pdf->SetXY(115, 33);
$pdf->MultiCell(90,5,"Nível de Alteração Eng.: $eng");
$pdf->SetXY(115, 38);
$pdf->MultiCell(90,5,"Nome do Laboratório: $local",1);

//linha 3
$pdf->SetFont('Arial','B',7.5);
$pdf->SetXY(5, 43);
$pdf->MultiCell(55,10,"Material Spec. No. / Rev / Data",1,'C');
$pdf->SetXY(60, 43);
$pdf->MultiCell(31,5,"Limites",1,'C');
$pdf->SetXY(60, 48);
$pdf->MultiCell(15.5,5,"LIE",1,'C');
$pdf->SetXY(75.5, 48);
$pdf->MultiCell(15.5,5,"LSE",1,'C');
$pdf->SetXY(91, 43);
$pdf->MultiCell(12,5,"Data do\nTeste",1,'C');
$pdf->SetXY(103, 43);
$pdf->MultiCell(12,5,"Quant\nTestada",1,'C');
$pdf->SetXY(115, 43);
$pdf->MultiCell(72,10,"Resultados das Medições pelo Fornecedor",1,'C');
$pdf->SetXY(187, 43);
...
//linha 4
$y=53;
$sql=mysql_query("SELECT ensaio1.*,car.numero,ensaio.local,car.descricao,car.simbolo,car.espec,car.tipo,car.lie,car.lse FROM apqp_car AS car, apqp_enma AS ensaio, apqp_enmal AS ensaio1 WHERE ensaio1.car=car.id AND ensaio.peca='$pc' AND ensaio.id=ensaio1.ensaio ORDER BY car.numero ASC");
if(mysql_num_rows($sql)){
	while($res=mysql_fetch_array($sql)){
		...
		$tam1[1]=flinha("$res[lie]",15.5);
		$tam1[2]=flinha("$res[lse]",15.5);
		...
		if(!empty($res["lie"])){
			$pdf->SetXY(60,$y);
			$pdf->MultiCell(15.5,$w,"$res[lie]",0,'C');
		}
		if(!empty($res["lse"])){
			$pdf->SetXY(75.5,$y);
			$pdf->MultiCell(15.5,$w,"$res[lie]",0,'C');
...
$pdf->SetFont('Arial','',7);
$pdf->SetXY(105, 254);
$pdf->MultiCell(100,5,"As informações gerais não conforme são inaceitáveis para todo o resultado dos testes.",1);

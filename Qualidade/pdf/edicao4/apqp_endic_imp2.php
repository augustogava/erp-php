...
if(mysql_num_rows($sql)){
	while($res=mysql_fetch_array($sql)){
		if($y>=250){
			...
			$pdf->MultiCell(104,15,"",1);
			$pdf->SetXY(5, 28);
			$pdf->MultiCell(110,5,"Facilidade da Inspeção: \n $local");
			$pdf->SetXY(109, 23);
			$pdf->MultiCell(90,5,"Nome da Peça: $nome");
			$pdf->SetXY(109, 28);
			$pdf->MultiCell(90,5,"Nível de alteração do projeto: $proj");
		...
		$tam1[2]=flinha("$res[lie]",12.5);
		$tam1[3]=flinha("$res[lse]",12.5);
		$tam1[4]=flinha("$data_teste",13);
		$tam1[5]=flinha("$res[quant_test]",12);
		$tam1[6]=1;
		$tam1[7]=flinha($res["forn"],40);
		if($tam1[7]==0){$tam1[7]=1;}
		$tam1[8]=flinha($res["cli"],40);
		if($tam1[8]==0){$tam1[8]=1;}
		$tam1[9]=1;
		$maxu=max($tam1);
		for($i=0;$i<=11;$i++){
			$ql[$i]=$maxu - $tam1[$i];

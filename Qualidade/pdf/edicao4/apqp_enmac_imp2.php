...
$pdf->SetXY(109, 38);
$pdf->MultiCell(96,5,"Nome do Laboratório: $local",1);
...
//
$pdf->SetXY(60, 43);
$pdf->MultiCell(25,5,"Limites",1,'C');
$pdf->SetXY(60, 48);
...
$pdf->SetXY(109, 43);
$pdf->MultiCell(80,5,"Resultados das Medições (Data)",1,'C');
$pdf->SetXY(109, 48);
...
//		
		$tam1[1]=flinha("$res[lie]",12.5);
		$tam1[2]=flinha("$res[lse]",12.5);
//
		$tam1[3]=flinha("$data_teste",13);
		$tam1[4]=flinha("$res[quant_test]",12);
		$tam1[5]=1;
		$tam1[6]=flinha($res["forn"],40);
		if($tam1[6]==0){$tam1[6]=1;}
		$tam1[7]=flinha($res["cli"],40);
		if($tam1[7]==0){$tam1[7]=1;}
		$tam1[8]=1;
		$maxu=max($tam1);
		for($i=0;$i<=11;$i++){
			$ql[$i]=$maxu - $tam1[$i];

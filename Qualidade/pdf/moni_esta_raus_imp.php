...
$pdf->Line(24,19,205,19);
    ...
    // Título
	$pdf->SetXY(5,12);
	$pdf->MultiCell(205,7,"RELATÓRIO DE ACESSO POR USUÁRIO",0,"C");
...
$sql=mysql_query("SELECT * FROM log WHERE data>='$data_i' AND data<='$data_f' AND user='$cliente' ORDER BY data ASC");

// alerta caso não exista registro armazenado
if(!mysql_num_rows($sql)){
	$_SESSION["mensagem"]="Não achou nenhum Registro";
	header("Location:../moni_esta_raus.php");
	exit;
}
...
while($res=mysql_fetch_array($sql)){
		// - - - - -- - - - - - -  -- - - - - - - - - - - - - - - - - 
			if($y>=265){
				...
				// 2ª linha  ////////////////////////////////////////////////////////
				$sql2=mysql_query("SELECT * FROM funcionarios WHERE id='$cliente'");
//print "SELECT * FROM funcionarios WHERE id='$cliente'";
				$res2=mysql_fetch_array($sql2);

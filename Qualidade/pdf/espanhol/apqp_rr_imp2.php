...
for($i=1;$i<=10;$i++){
	if(y>=250){
		...
		$pdf->MultiCell(80,5,"Proveedor\n $razao",1);
		$pdf->SetXY(85, 28);
		$pdf->MultiCell(60,5,"Número de la Pieza (Proveedor) \n $num",1);
		$pdf->SetXY(145, 28);
		$pdf->MultiCell(60,5,"Revisión de la Pieza (Proveedor) \n $revi",1);
		//linha 3
		$pdf->SetXY(5, 38);
...
if($y>=180){
	...
	$pdf->SetXY(70, 18);
	$pdf->MultiCell(70,5,"Númeor de la Pieza (Cliente) \n $peca",1);
	$pdf->SetXY(140, 18);
	...
	$pdf->SetXY(105, 28);
	$pdf->MultiCell(100,5,"Npumero de la Pieza (Proveedor) \n $nome",1);
	//linha 3
	$pdf->SetXY(5, 38);
...
$pdf->SetXY(5, $y);
if($resp["sit"]==0){ $ok="pendente"; }elseif($resp["sit"]==1){ $ok="approvado"; }else{ $ok="reprovado"; }

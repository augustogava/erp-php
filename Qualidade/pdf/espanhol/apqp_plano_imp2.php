...
$pdf->SetXY(5, 4);
$pdf->MultiCell(290,5,"PLAN DE CONTROL",0,'C');
$pdf->SetFont('Arial','',8);
$pdf->SetXY(260, 8);
$pg=1;
$pdf->SetXY(260, 5);
...
$pdf->SetXY(195, 18);
$pdf->MultiCell(30,10,"Prototipo");
if($resp["fase"]==2){ $fig2="2"; }else{ $fig2="1"; }
$pdf->Image("imagens/icon14_check$fig2.jpg",220,20,4,4);
$pdf->SetXY(225, 18);
$pdf->MultiCell(30,10,"Pré-Lanzamiento");
if($resp["fase"]==3){ $fig3="2"; }else{ $fig3="1"; }
$pdf->Image("imagens/icon14_check$fig3.jpg",250,20,4,4);
$pdf->SetXY(255, 18);
$pdf->MultiCell(30,10,"Producción");
//linha 2
$pdf->SetXY(5, 28);
$pdf->MultiCell(35,5,"No. Plan Control \n $resp[numero]",1);
$pdf->SetXY(40, 28);
$pdf->MultiCell(95,5,"Nombre de la Pieza \n $res[nome]",1);
$pdf->SetXY(135, 28);
$pdf->MultiCell(90,5,"Proveedor \n $rese[razao]",1);
$pdf->SetXY(225, 28);
$pdf->MultiCell(30,5,"Cód. Proveedor \n ",1);
$pdf->SetXY(255, 28);
$pdf->MultiCell(35,5,"Fecha (Rev.) \n ".banco2data($resp["rev"])."",1);
//linha 3
$pdf->SetXY(5, 38);
$pdf->MultiCell(130,5,"Equipo Principal \n $resp[equipe]",1);
$pdf->SetXY(135, 38);
$pdf->MultiCell(50,5,"Proveedor/Aprobación Planta/Fecha  \n ".$resp["quem"]." / ".banco2data($resp["dtquem"])."",1);
$pdf->SetXY(185, 38);
$pdf->MultiCell(70,5,"Número/Rev. Pieza (Proveedor) \n ".$res["numero"]." - ".$res["rev"]."",1);
$pdf->SetXY(255, 38);
$pdf->MultiCell(35,5,"Fecha(Orig.) \n ".banco2data($resp["ini"])."",1);
//linha 4
$pdf->SetXY(5, 48);
$pdf->MultiCell(130,5,"Aprobación Ingeniería Cliente/Fecha (Si se necesita) \n ".$resp["apro1"]." / ".banco2data($resp["apro1_data"])."",1);
$pdf->SetXY(135, 48);
$pdf->MultiCell(90,5,"Aprobación Calidad Cliente/Fecha (Si se necesita) \n ".$resp["apro2"]." / ".banco2data($resp["apro2_data"])."",1);
$pdf->SetXY(225, 48);
$pdf->MultiCell(65,5,"Persona de Contacto/Teléfono \n $resp[contato]",1);
//linha 5
$pdf->SetXY(5, 58);
$pdf->MultiCell(130,5,"Otras Aprovaciones/Fecha(Si se necesita)  \n ".$resp["apro3"]." / ".banco2data($resp["apro3_data"])."",1);
$pdf->SetXY(135, 58);
$pdf->MultiCell(155,5,"Otras Aprobaciones/Fecha (Si se necesita)   \n ".$resp["apro4"]." / ".banco2data($resp["apro4_data"])."",1);
//linha 6
$pdf->SetXY(5, 70);
$pdf->MultiCell(15,5,"Número Pieza/Proceso",1,'C');
$pdf->SetXY(20, 70);
$pdf->MultiCell(35,5,"Nombre Proceso/Descripción de la Operación",1,'C');
$pdf->SetXY(55, 70);
$pdf->MultiCell(35,5,"Maquina Utilizada/Herramienta \n ",1,'C');
$pdf->SetXY(90, 70);
$pdf->MultiCell(50,10,"Características",1,'C');
	$pdf->SetXY(90, 80);
	$pdf->MultiCell(6,5,"Nº",1,'C');
	$pdf->SetXY(96, 80);
	$pdf->MultiCell(20,5,"Producto",1,'C');
	$pdf->SetXY(116, 80);
	$pdf->MultiCell(24,5,"Proceso",1,'C');
$pdf->SetXY(140, 70);
$pdf->MultiCell(5,5,"C a r",1,'C');
$pdf->SetXY(145, 70);
$pdf->MultiCell(115,5,"Método",1,'C');
	$pdf->SetXY(145, 75);
	$pdf->MultiCell(30,5,"Tol./Espec. Del Prod./Proc.",1,'C');
	$pdf->SetXY(175, 75);
	$pdf->MultiCell(25,5,"Técnica Eval./Medida ",1,'C');
	$pdf->SetXY(200, 75);
	$pdf->MultiCell(30,5,"Muestro",1,'C');
		$pdf->SetXY(200, 80);
		$pdf->MultiCell(15,5,"Tam.",1,'C');
		$pdf->SetXY(215, 80);
		$pdf->MultiCell(15,5,"Freq.",1,'C');
	$pdf->SetXY(230, 75);
	$pdf->MultiCell(30,10,"Método de Control",1,'C');
$pdf->SetXY(260, 70);
$pdf->MultiCell(30,5,"Plan de Reacción \n acciones correctivas \n ",1,'C');
...
if(mysql_num_rows($sql)){
	...
	while($res=mysql_fetch_array($sql)){
		...
		if($y>=185){
			...
			$pdf->MultiCell(35,5,"Nombre Proceso/Descripción de la Operación",1,'C');
			$pdf->SetXY(55, 30);
			$pdf->MultiCell(35,5,"Maquina Utilizada/Herramienta",1,'C');
			$pdf->SetXY(90, 30);
			$pdf->MultiCell(50,10,"Características",1,'C');
				$pdf->SetXY(90, 40);
				$pdf->MultiCell(6,5,"Nº",1,'C');

...
//linha 4
$sql=mysql_query("SELECT ensaio1.*,car.numero,car.descricao,car.simbolo,car.espec,car.tipo FROM apqp_car AS car, apqp_ende AS ensaio, apqp_endel AS ensaio1 WHERE ensaio1.car=car.id AND ensaio.peca='$pc' AND ensaio.id=ensaio1.ensaio");
$y=50;
if(mysql_num_rows($sql)){
	while($res=mysql_fetch_array($sql)){
		...
		$tam1[0]=flinha($res["numero"],2);
		if($tam1[0]==0){$tam1[0]=1;}
		$tam1[1]=flinha($res["ref"],8);
		if($tam[1]==0){$tam1[1]=1;}
		$tam1[2]=flinha($res["descricao"]." - ".$res["espec"],80);
		if($tam[2]==0){$tam1[2]=1;}
		$tam1[3]=flinha($res["freq"],4);
		if($tam[3]==0){$tam1[3]=1;}
		$tam1[4]=flinha($res["qtd"],8);
		if($tam[4]==0){$tam1[4]=1;}
		$tam1[5]=flinha($res["forn"],60);
		if($tam[5]==0){$tam1[5]=1;}
		$maxu=max($tam1);
		for($i=0;$i<=11;$i++){
			$ql[$i]=$maxu - $tam1[$i];
		...
		$pdf->SetXY(15, $y);
		$pdf->MultiCell(15,$w,"$res[ref]",0,'C');
		$pdf->SetXY(30, $y);
		$pdf->MultiCell(75,$w,"$res[descricao] - $res[espec]",0);
		$pdf->SetXY(105, $y);
		$pdf->MultiCell(15,$w,"$res[freq]",0,'C');
		$pdf->SetXY(120, $y);
		$pdf->MultiCell(10,$w,"$res[qtd]",0,'C');
		$pdf->SetXY(130, $y);
		$pdf->MultiCell(55,$w,"$res[forn]",0);
		$pdf->SetXY(185, $y);
		if($res["ok"]=="S") $ok="X";

...
$pdf->Cell(70);
$pdf->Cell(0, 18, 'INFORME DE APROBACION DE APARIENCIA');
$pdf->SetFont('Arial','',8);
$pdf->SetXY(5, 18);
...
$pdf->SetXY(135, 18);
$pdf->MultiCell(60,5,"Número del Plano \n $res[desenhoc]",1);
$pdf->SetXY(195, 18);
$pdf->MultiCell(95,5,"Aplicación (Vehículos) \n $res[aplicacao]",1);
$pdf->SetXY(260, 5);
...
//linha 2
$pdf->SetXY(5, 28);
$pdf->MultiCell(75,5,"Nombre de la Pieza \n $res[nome]",1);
$pdf->SetXY(80, 28);
$pdf->MultiCell(75,5,"Código del comprador \n $resp[comprador]",1);
$pdf->SetXY(155, 28);
$pdf->MultiCell(105,5,"Nível E/C \n $res[niveleng]",1);
$pdf->SetXY(260, 28);
$pdf->MultiCell(30,5,"Fecha \n ".banco2data($res["dteng"])."",1);
//linha3
$pdf->SetXY(5, 38);
$pdf->MultiCell(100,5,"Nombre del Proveedor \n a$rese[razao]",1);
$pdf->SetXY(105, 38);
$pdf->MultiCell(50,5,"Número de la Pieza (Subministrador) \n  $res[numero] - $res[rev]",1);
$pdf->SetXY(155,38);
$pdf->MultiCell(105,5,"Lugar de la Fabricación \n $resp[local]",1);
$pdf->SetXY(260, 38);
$pdf->MultiCell(30,5,"Código del Proveedor $resp[seilaquecodigoehesse]",1);
//linha4
	//coluna 1
	$pdf->SetXY(5, 48);
	...
	//linha 4-1-1
		$pdf->SetXY(5, 48);
		$pdf->MultiCell(40,5,"Razón para la presentación");
		$pdf->SetXY(45, 49);
		if($resp["razao1"]=="1"){ $show1="X"; }else{ $show1=" "; }
		$pdf->MultiCell(5,5,"$show1",1,'C');
		$pdf->SetXY(50, 48);
		$pdf->MultiCell(40,5,"Certificado Presentacion Piezas");
		$pdf->SetXY(90, 49);
		if($resp["razao1"]=="2"){ $show2="X"; }else{ $show2=" "; }
		$pdf->MultiCell(5,5,"$show2",1,'C');
		$pdf->SetXY(95, 48);
		$pdf->MultiCell(40,5,"Mostra Especial");
		$pdf->SetXY(135, 49);
	...
	//linha 4-1-2
		...
		$pdf->SetXY(95, 58);
		$pdf->MultiCell(40,5,"Embarque da Primeira Production");
		$pdf->SetXY(135, 59);
...
//linha 5 
	$pdf->SetXY(5, 70);
	$pdf->SetFont('Arial','B',8);
	$pdf->MultiCell(285,5,"AVALUACION DE APARIENCIA",1,'C');
	$pdf->SetXY(5, 75);
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(185,5,"Informacion de Fuentes Y Textura del Subministrador",1,'C');
	$pdf->SetXY(5, 80);
	...
	//tabela direta
	$pdf->SetXY(190, 75);
	$pdf->MultiCell(40,10,"Evaluación Pré-Textura",1,'C');
	$pdf->SetXY(230, 75);
	$pdf->MultiCell(60,5,"Firma del Representante del \n Cliente Y Fecha ",1,'C');
	$pdf->SetXY(190, 85);
	$pdf->MultiCell(40,10,"Correcto Y comenzar",1,'C');
	$pdf->SetXY(230, 85);
	$pdf->MultiCell(60,10," ",1,'C');
	$pdf->SetXY(190, 95);
	$pdf->MultiCell(40,10,"Correcto Y Reenviar",1,'C');
	$pdf->SetXY(230, 95);
	$pdf->MultiCell(60,10,"",1,'C');
	$pdf->SetXY(190, 105);
	$pdf->MultiCell(40,5,"Aprobado para textura",1,'C');
	$pdf->SetXY(230, 105);
...
//linha 6
	...
	$pdf->SetXY(5, 112);
	$pdf->SetFont('Arial','B',8);
	$pdf->MultiCell(15,5,"Sufijo del\n Color \n ",1,'C');
	$pdf->SetXY(20, 112);
	$pdf->MultiCell(35,10,"Dados tricotomia",1,'C');
	$pdf->SetXY(55, 112);
	$pdf->MultiCell(15,5,"Nº del \n\n Patron",1,'C');
	$pdf->SetXY(70, 112);
	$pdf->MultiCell(20,5,"Fecha del \n\nPatron",1,'C');
	$pdf->SetXY(90, 112);
	$pdf->MultiCell(25,15,"Tipo del Material",1,'C');
	$pdf->SetXY(115, 112);
	$pdf->MultiCell(30,15,"Origen del Material",1,'C');
	$pdf->SetXY(145, 112);
	$pdf->MultiCell(35,10,"Colorido",1,'C');
	$pdf->SetXY(180, 112);
	$pdf->MultiCell(20,10,"Tono",1,'C');
	$pdf->SetXY(200, 112);
	$pdf->MultiCell(20,10,"Croma",1,'C');
	$pdf->SetXY(220, 112);
	$pdf->MultiCell(20,10,"Brillo",1,'C');
	$pdf->SetXY(240, 112);
	$pdf->MultiCell(20,5,"Brillo \n Metállico",1,'C');
	$pdf->SetXY(260, 112);
	$pdf->MultiCell(15,5,"Sufijo \n del color de envio",1,'C');
	$pdf->SetXY(275, 112);
	$pdf->MultiCell(15,5,"Dispo. \n de la Pieza  ",1,'C');
	//linha 2
	$pdf->SetXY(20, 122);
	$pdf->SetFont('Arial','',7);
	...
	while($res=mysql_fetch_array($sql)){
		if($y>=185){
			...
			//linha 5 
				$pdf->SetXY(5, 18);
				$pdf->SetFont('Arial','B',8);
				$pdf->MultiCell(285,5,"EVALUATION DE APARIENCIA",1,'C');
				$pdf->SetXY(5, 23);
				$pdf->SetFont('Arial','',8);
				$pdf->MultiCell(185,5,"Informacion de Fuentes Y Textura del Subministrador",1,'C');
				$pdf->SetXY(5, 28);

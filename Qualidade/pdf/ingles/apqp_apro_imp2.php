...
$pdf->Cell(70);
$pdf->Cell(0, 18, 'APPEARANCE APPROVAL REPORT');
$pdf->SetFont('Arial','',8);
$pdf->SetXY(5, 18);
$pdf->MultiCell(60,5,"Customer \n $res[nomecli]",1);
$pdf->SetXY(65, 18);
$pdf->MultiCell(70,5,"Part Number(Customer) \n $res[pecacli]",1);
$pdf->SetXY(135, 18);
$pdf->MultiCell(60,5,"Drawing Number \n $res[desenhoc]",1);
$pdf->SetXY(195, 18);
$pdf->MultiCell(95,5,"Application(Vehicles) \n $res[aplicacao]",1);
$pdf->SetXY(260, 5);
...
//linha 2
$pdf->SetXY(5, 28);
$pdf->MultiCell(75,5,"Part Name \n $res[nome]",1);
$pdf->SetXY(80, 28);
$pdf->MultiCell(75,5,"Buyer \n $resp[comprador]",1);
$pdf->SetXY(155, 28);
$pdf->MultiCell(105,5,"Engineering Change Level \n $res[niveleng]",1);
$pdf->SetXY(260, 28);
...
$pdf->SetXY(105, 38);
$pdf->MultiCell(50,5,"Supplier Part Number \n  $res[numero] - $res[rev]",1);
$pdf->SetXY(155,38);
$pdf->MultiCell(105,5,"Manufacturing location \n $resp[local]",1);
$pdf->SetXY(260, 38);
...
//linha 5 
	$pdf->SetXY(5, 70);
	$pdf->SetFont('Arial','B',8);
	$pdf->MultiCell(285,5,"APPEARANCE EVALUATION",1,'C');
	$pdf->SetXY(5, 75);
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(185,5,"SUPPLIER SOURCING AND TEXTURE INFORMATION",1,'C');
	$pdf->SetXY(5, 80);
	...
	//tabela direta
	$pdf->SetXY(190, 75);
	$pdf->MultiCell(40,10,"Pre-texture Evaluation",1,'C');
	$pdf->SetXY(230, 75);
	$pdf->MultiCell(60,5,"Customer Representative Signature And Date ",1,'C');
	$pdf->SetXY(190, 85);
	$pdf->MultiCell(40,10,"Corret and proceed",1,'C');
	$pdf->SetXY(230, 85);
	$pdf->MultiCell(60,10," ",1,'C');
	$pdf->SetXY(190, 95);
	$pdf->MultiCell(40,10,"Correct and Resubmit",1,'C');
	$pdf->SetXY(230, 95);
	$pdf->MultiCell(60,10,"",1,'C');
	$pdf->SetXY(190, 105);
	$pdf->MultiCell(40,5,"Approved to Texture",1,'C');
	$pdf->SetXY(230, 105);
...
//linha 6
	...
	$pdf->SetXY(5, 112);
	$pdf->SetFont('Arial','B',8);
	$pdf->MultiCell(15,5,"Color \n Suffix \n ",1,'C');
	$pdf->SetXY(20, 112);
	$pdf->MultiCell(35,10,"Tristimulus Data",1,'C');
	$pdf->SetXY(55, 112);
	$pdf->MultiCell(15,5," \n Master Number \n",1,'C');
	$pdf->SetXY(70, 112);
	$pdf->MultiCell(20,5," \n Master Date \n ",1,'C');
	$pdf->SetXY(90, 112);
	$pdf->MultiCell(25,15,"Material Type",1,'C');
	$pdf->SetXY(115, 112);
	$pdf->MultiCell(30,15,"Material Source",1,'C');
	$pdf->SetXY(145, 112);
	$pdf->MultiCell(35,10,"Tonality",1,'C');
	$pdf->SetXY(180, 112);
	$pdf->MultiCell(20,10,"Value",1,'C');
	$pdf->SetXY(200, 112);
	$pdf->MultiCell(20,10,"Chroma",1,'C');
	$pdf->SetXY(220, 112);
	$pdf->MultiCell(20,10,"Gloss",1,'C');
	$pdf->SetXY(240, 112);
	$pdf->MultiCell(20,5,"Metallic \n Brilhance",1,'C');
	$pdf->SetXY(260, 112);
	$pdf->MultiCell(15,5,"Color \n Shipp. Suffix",1,'C');
	$pdf->SetXY(275, 112);
	$pdf->MultiCell(15,5,"Part \n Dispos. \n ",1,'C');
	//linha 2
	$pdf->SetXY(20, 122);
	$pdf->SetFont('Arial','',7);
	...
	$pdf->SetXY(145, 122);
	$pdf->MultiCell(10,5,"Red",1,'C');
	$pdf->SetXY(155, 122);
	$pdf->MultiCell(8,5,"Yel",1,'C');
	$pdf->SetXY(163, 122);
	$pdf->MultiCell(8,5,"Gree",1,'C');
	$pdf->SetXY(171, 122);
	$pdf->MultiCell(9,5,"Blue",1,'C');
	$pdf->SetXY(180, 122);
	$pdf->MultiCell(10,5,"Light",1,'C');
	$pdf->SetXY(190, 122);
	$pdf->MultiCell(10,5,"Dark",1,'C');
	$pdf->SetXY(200, 122);
	$pdf->MultiCell(10,5,"Gray",1,'C');
	$pdf->SetXY(210, 122);
	$pdf->MultiCell(10,5,"_impc",1,'C');
	$pdf->SetXY(220, 122);
	$pdf->MultiCell(10,5,"High",1,'C');
	$pdf->SetXY(230, 122);
	$pdf->MultiCell(10,5,"Low",1,'C');
	$pdf->SetXY(240, 122);
	$pdf->MultiCell(10,5,"High",1,'C');
	$pdf->SetXY(250, 122);
	$pdf->MultiCell(10,5,"Low",1,'C');
	...
	while($res=mysql_fetch_array($sql)){
		if($y>=185){
			...
			//linha 5 
				...
				$pdf->MultiCell(60,5," Customer Representative Signature and Date ",1,'C');
				$pdf->SetXY(190, 33);
				$pdf->MultiCell(40,10,"Correct and Proceed",1,'C');
				$pdf->SetXY(230, 33);
				$pdf->MultiCell(60,10," ",1,'C');
				$pdf->SetXY(190, 43);
				$pdf->MultiCell(40,10,"Corret and Resubmit",1,'C');
				$pdf->SetXY(230, 43);
				$pdf->MultiCell(60,10,"",1,'C');
				$pdf->SetXY(190, 53);
				$pdf->MultiCell(40,5,"Approved to texture",1,'C');

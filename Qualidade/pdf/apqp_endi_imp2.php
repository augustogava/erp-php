...
$pdf->SetXY(107, 40);
$pdf->MultiCell(80,10,"Resultados das Medições pelo Fornecedor ",1,'C');
$pdf->SetXY(187, 40);
...
if(mysql_num_rows($sql)){
	while($res=mysql_fetch_array($sql)){
		...
		$tam1[0]=flinha($res["numero"],6);
		$tam1[2]=flinha("$res[descricao] - $res[espec]",50);
		$tam1[3]=1;
		$tam1[4]=flinha($res["forn"],40);
		...
		$pdf->SetXY(60,$y);
		if(!empty($res2["lie"])&& !empty($res2["lse"])){
			$pdf->MultiCell(31,$w,"$res2[lie] / $res2[lse]",0);
		}
		$pdf->SetXY(95, $y);
